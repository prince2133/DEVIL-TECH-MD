...
const sessdata = config.SESSION_ID.replace("DEVIL-TECH-MD~", '');
...
const port = process.env.PORT || 9090;
  ...
  let up = `*𝐇𝐄𝐘 👋🏻 𝐋𝐄𝐆𝐄𝐍𝐃 𝐃𝐄𝐕𝐈𝐋 𝐓𝐄𝐂𝐇 𝐌𝐃 𝐁𝐎𝐓*
...
│• 🌸𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑 : ᴄʏʙᴇʀ ʟᴏᴋᴜ ᴀꜱʜᴜᴜ ᴏꜰᴄ
...
if (!isReact && senderNumber === botNumber) {
    if (config.CUSTOM_OWNER_REACT === 'true') {
        // Use custom emojis from the configuration
        const reactions = (config.CUSTOM_OWNER_REACT || '🥲,😂,👍🏻,🙂,😔').split(',');
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
        m.react(randomReaction);
    }
} 
