...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .ᴄᴜꜱᴛᴏᴍ-ʀᴇᴀᴄᴛ ᴏɴ*`);
    }
});
...
//--------------------------------------------
cmd({
    pattern: "custom-owner-react",
    alias: ["customownerreact"],
    desc: "Enable or disable the autoreact feature",
    category: "settings",
    filename: __filename
},    
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    // Check the argument for enabling or disabling the anticall feature
    if (args[0] === "on") {
        config.CUSTOM_OWNER_REACT = "true";
        await reply("*autoreact feature is now enabled.*");
    } else if (args[0] === "off") {
        config.CUSTOM_OWNER_REACT = "false";
        await reply("autoreact feature is now disabled.");
    } else {
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .ᴀᴜᴛᴏ-ʀᴇᴀᴄᴛ ᴏɴ*`);
    }
...
cmd({
  ...
  try {
    ...
    if (args[0] === "on") {
      ...
      await reply(`*Invalid input! Use either 'on' or 'off'. Example: antidelete on*`);
    }
  } catch (error) {
    return reply(`*An error occurred while processing your request.*\n\n_Error:_ ${error.message}`);
  }
